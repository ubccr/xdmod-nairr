...
WITH RankedRequests AS (
  SELECT
      ...
      r.date_submitted,
      ROW_NUMBER() OVER (
PARTITION BY rm.request_number
ORDER BY rpr.begin_date DESC
) AS row_rank
  FROM xras.people p
           JOIN xras.organizations o ON o.organization_id = p.organization_id
...
hierarchy_sql="""
select ft1.fos_name, ft1.fos_type_parent_id, parent_ft.fos_name as parent_name
from xras.fos_types ft1
LEFT JOIN xras.fos_types parent_ft ON parent_ft.fos_type_id = ft1.fos_type_parent_id
WHERE ft1.allocations_process_id = 108
...
WHERE ft.allocations_process_id = 108
AND rm.request_number IS NOT NULL
ORDER BY rm.request_number ASC;
    ...
    with psycopg2.connect(**db_config) as conn:
        ...
        with conn.cursor() as cur:
            ...
            fetch_and_append(
                cur,
                names_sql,
                lambda row : [row[0], row[1], row[3], row[4]],
                names
            )
            ...
            fetch_and_append(
                cur,
                hierarchy_sql,
                lambda row : [row[0], row[0], row[2] or ''],
                fos_list
            )
